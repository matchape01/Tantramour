/**
 * TANTRAMOUR 2026 — Informations Logistiques Spéciales (Supplémentaires)
 * =======================================================================
 * Structure : LOGISTICS_SPECIAL = { "<logisticId>": { html, text, updatedAt } }
 */

var LOGISTICS_SPECIAL = {

  "JOUR1_OPENING_CEREMONY": {
    html: "",
    text: "",
    updatedAt: "2026-08-29 14:37"
  },

  "JOUR1_SOIREE_ECSTATIC_DANSE": {
    html: "2 helpers ayant de l'expérience en danse extatique (ou danse consciente) qui puissent jouer le rôle de « gardiens de l'espace »",
    text: "2 helpers ayant de l'expérience en danse extatique (ou danse consciente) qui puissent jouer le rôle de « gardiens de l'espace »",
    updatedAt: "2026-08-29 15:16"
  },

  "JOUR1_VERIFICATION_STOCK_EQUIPEMENTS": {
    html: "",
    text: "",
    updatedAt: "2026-08-26 15:08"
  },

  "JOUR1_VERIFICATION_STOCK_MATOS_HAMEA": {
    html: "",
    text: "",
    updatedAt: "2026-08-24 01:21"
  },

  "JOUR2_ALICE___OUVERTURE__1_3____MASS": {
    html: "micro casque - <br>matelas auto gonflable, ramené par Joe",
    text: "micro casque -\nmatelas auto gonflable, ramené par Joe",
    updatedAt: "2026-08-29 13:07"
  },

  "JOUR2_CESSER_DE_FAIRE__LAISSER_ETRE": {
    html: "non je sais pas pour le moment",
    text: "non je sais pas pour le moment",
    updatedAt: "2026-08-25 03:37"
  },

  "JOUR2_DANSE_ECSTATIC_CONTACT__1_": {
    html: "",
    text: "",
    updatedAt: ""
  },

  "JOUR2_INMEN___HELD_BY_MEN_A_SOMATIC_": {
    html: "",
    text: "",
    updatedAt: ""
  },

  "JOUR2_LA_DANSE_DE_LA_POLARITE": {
    html: "",
    text: "",
    updatedAt: ""
  },

  "JOUR2_LA_SAVEUR_DU_DESIR_": {
    html: "- 6 assiettes <br>- 2 tables<br>(J'amène moi-même les besoins spécifiques)",
    text: "- 6 assiettes \n- 2 tables\n(J'amène moi-même les besoins spécifiques)",
    updatedAt: "2026-08-26 18:36"
  },

  "JOUR2_L_UNION_SACREE_D_HATHOR_ET_HOR": {
    html: "huile de massage, coupelles, du sopalin et du gel hydro, boite mouchoirs, bougies blanches (2 grandes et une 10aine type chauffe plats)",
    text: "huile de massage, coupelles, du sopalin et du gel hydro, boite mouchoirs, bougies blanches (2 grandes et une 10aine type chauffe plats)",
    updatedAt: "2026-08-26 18:36"
  },

  "JOUR2_MEETING_YOUR_DEITY___HYPNOSIS_": {
    html: "sono <br>micro sans fil, <br>matériel artistique (bâtons de colle, papier épais, feutres/feutres de couleur, morceaux de magazines, tissus, etc.)",
    text: "sono \nmicro sans fil, \nmatériel artistique (bâtons de colle, papier épais, feutres/feutres de couleur, morceaux de magazines, tissus, etc.)",
    updatedAt: "2026-08-26 18:36"
  },

  "JOUR2_MOVE___CONNECT_WITH_THE_5_ELEM": {
    html: "",
    text: "",
    updatedAt: ""
  },

  "JOUR2_OPENING_CONNECTIONS": {
    html: "1 angel formé à la prise en charge des traumatismes ?<br>6 spots lumineux (à voir avec Félix)<br>J’aimerais peut-être aussi que quelqu’un m’aide à modifier l’éclairage si possible, mais je ne le saurai qu’après avoir vu les lieux",
    text: "1 angel formé à la prise en charge des traumatismes ?\n6 spots lumineux (à voir avec Félix)\nJ’aimerais peut-être aussi que quelqu’un m’aide à modifier l’éclairage si possible, mais je ne le saurai qu’après avoir vu les lieux",
    updatedAt: "2026-08-30 10:44"
  },

  "JOUR2_PREPARATION_RASSEMBLEMENT": {
    html: "enscens / bougies",
    text: "enscens / bougies",
    updatedAt: "2026-08-28 15:26"
  },

  "JOUR2_ROUE_DE_TERRE___CONSENTIR_ET_C": {
    html: "diffusion son bluetooth",
    text: "diffusion son bluetooth",
    updatedAt: "2026-08-25 03:37"
  },

  "JOUR2_SHAKTI_PUJA___LE_MYSTERE_DES_M": {
    html: "Lingettes humides - 3kg de riz blanc (le moins chere possible)",
    text: "Lingettes humides - 3kg de riz blanc (le moins chere possible)",
    updatedAt: "2026-08-25 03:37"
  },

  "JOUR2_SHIBARI_PERFORMANCE___INTERACT": {
    html: "mise en place point de suspension ( je prends soin de cela)<br>60 tapis de yoga<br>12 spots (Voir avec Felix)",
    text: "mise en place point de suspension ( je prends soin de cela)\n60 tapis de yoga\n12 spots (Voir avec Felix)",
    updatedAt: "2026-08-29 16:52"
  },

  "JOUR2_VIBRATION___L_UNISSON__T_TE___": {
    html: "MATOS: Spots qui enveloppent toute la pièce de Lumière orangée douce (Voir avec Felix) - Quantité: 6<br>STAFF: 1 Helper, 1 Angel et 1 traducteur =&gt; A valider avec Sabryna/Chris",
    text: "MATOS: Spots qui enveloppent toute la pièce de Lumière orangée douce (Voir avec Felix) - Quantité: 6\nSTAFF: 1 Helper, 1 Angel et 1 traducteur => A valider avec Sabryna/Chris",
    updatedAt: "2026-08-25 03:37"
  },

  "JOUR2_YABYUM___LET_LOVE_HAPPEN": {
    html: "• Sono &amp; table de mix<br>• Micro main sans fil<br>• Tapis de yoga : 1 pour 2 <br>• BEAUCOUP de coussins : au moins 2 par personne",
    text: "• Sono & table de mix\n• Micro main sans fil\n• Tapis de yoga : 1 pour 2 \n• BEAUCOUP de coussins : au moins 2 par personne",
    updatedAt: "2026-08-26 18:36"
  },

  "JOUR2__CALL__POUR_RASSEMBLEMENT": {
    html: "",
    text: "",
    updatedAt: "2026-08-25 10:18"
  },

  "JOUR2____STAND__MAQUILLAGE___": {
    html: "",
    text: "",
    updatedAt: "2026-08-24 01:27"
  },

  "JOUR3_ALCHIMIE_DU_COEUR__DANSE_M_DEC": {
    html: "- micro casque",
    text: "- micro casque",
    updatedAt: "2026-08-25 03:37"
  },

  "JOUR3_ANGEL___SHIFT_MATIN__AM_": {
    html: "<br>",
    text: "",
    updatedAt: "2026-08-28 15:27"
  },

  "JOUR3_DANCING_EROS": {
    html: "",
    text: "",
    updatedAt: ""
  },

  "JOUR3_DANSE_DES_PORTES_MULTIDIMENSIO": {
    html: "",
    text: "",
    updatedAt: "2026-08-25 21:01"
  },

  "JOUR3_DANSE_ORGASMIQUE___DANSER_LA_J": {
    html: "- 1 micro casque",
    text: "- 1 micro casque",
    updatedAt: "2026-08-25 03:37"
  },

  "JOUR3_ENTRE_DANS_LA_CARESSE_COMME_DA": {
    html: "25 Masques",
    text: "25 Masques",
    updatedAt: "2026-08-31 08:23"
  },

  "JOUR3_INITIATION_A__LA_DANSE_FUSION": {
    html: "Mon anglais est un peu limite.<br>Si un traducteur est dispo ça peut être pas mal, sinon je me débrouillerais.",
    text: "Mon anglais est un peu limite.\nSi un traducteur est dispo ça peut être pas mal, sinon je me débrouillerais.",
    updatedAt: "2026-08-26 18:36"
  },

  "JOUR3_LE_TOUCHER_COMME_PORTE_VERS_L_": {
    html: "-au centre : 30 petites coupelles + pots huile de coco (chacun prend une coupelle et y met de l'huile à son arrivée) + plusieurs cuillères<div><br></div><div>Préparation temple :<br>-autel au centre (tissus + statuette)</div>",
    text: "-au centre : 30 petites coupelles + pots huile de coco (chacun prend une coupelle et y met de l'huile à son arrivée) + plusieurs cuillères\n\n\nPréparation temple :\n-autel au centre (tissus + statuette)",
    updatedAt: "2026-08-30 00:19"
  },

  "JOUR3_LOVE_TEMPLE_DES_EROTYPES__R": {
    html: "Achats que nous apporterons, achats sur place et approts<br>Intro : danse métamorphique alchimique  (Ishvari ? Kalista ? )<br>Collant transparent blanc ou invisible : (en cours de recherche)<br>ARTISTE : peintures corps ou crayon<br>Crayon de couleurs pour peau : 11 €  (acheté par Helene ou Audrey)<br>Peinture (à valider par Helene et Karen : Helene les achetera)<br>Helene prend son APN (pas de frais photo)<br><br>Etheré : <br>Achats (par Paul ou Audrey, à livrer direct sur place si validé): <br>SPARKLERS CLUB Bouteille Gaz Hélium 50 Ballons, Bonbonne Jetable | Capacité 0,40m3 : env 50 € à valider<br>A valider avec Sabryna<br>Kit Arche Ballon Bleu Et Blanc, Bleu marine ciel métallisé argent paillette | 119 Bleu Ciel : 13 €<br>Apport lieu : <br>1 mini table basse pour autel au centre de l etoile de matelas<br>6 matelas / 12 coussins<br>2 Projecteurs : lumiere blanche et une lumière bleue<br>Apport perso <br>nos \"déités\"<br>Apport Tantramour<br>tissu blanc soyeux + tissu bleu argenté soyeux<br>1 boule à facette<br>1 guirlande lumineuse<br>1 aurasoma<br>1 grande bougie blanche (Led?)<br><br>Sensitif : <br>ACHATS : <br>Talc hypoallergenique (env . 5 € la boite en pharmaicie X2)<br>Plumes : 20€<br>Apports perso: <br>Foulards,<br>Tissus<br>Accessoires <br><br>Primal : <br>Achats : Argiles 4 couleurs : <br> CATTIER Argile verte ultra-ventilée 750g   : 11€<br>puis 3 autres couleurs en paquet de 600 gr : 7 € chaque X3<br>Matériel restaurant ou lingerie<br>* 8 bols (2 par couleur d'argile)<br>* des bouteilles d'eau pour faire les mélange d'argile et humidifier les tissus<br>* des linges types carrés de serviettes éponges ou gants de toilette,  pour les mains ou nettoyer une partie du corps si besoin<br>* des cuillères pour mélanger les préparations d'argile<br>1 bassine de rinçage (eau)<br><br>Soundsystem avec 3 micros : Audrey, Paul et Traducteur-ice",
    text: "Achats que nous apporterons, achats sur place et approts\nIntro : danse métamorphique alchimique  (Ishvari ? Kalista ? )\nCollant transparent blanc ou invisible : (en cours de recherche)\nARTISTE : peintures corps ou crayon\nCrayon de couleurs pour peau : 11 €  (acheté par Helene ou Audrey)\nPeinture (à valider par Helene et Karen : Helene les achetera)\nHelene prend son APN (pas de frais photo)\n\nEtheré : \nAchats (par Paul ou Audrey, à livrer direct sur place si validé): \nSPARKLERS CLUB Bouteille Gaz Hélium 50 Ballons, Bonbonne Jetable | Capacité 0,40m3 : env 50 € à valider\nA valider avec Sabryna\nKit Arche Ballon Bleu Et Blanc, Bleu marine ciel métallisé argent paillette | 119 Bleu Ciel : 13 €\nApport lieu : \n1 mini table basse pour autel au centre de l etoile de matelas\n6 matelas / 12 coussins\n2 Projecteurs : lumiere blanche et une lumière bleue\nApport perso \nnos \"déités\"\nApport Tantramour\ntissu blanc soyeux + tissu bleu argenté soyeux\n1 boule à facette\n1 guirlande lumineuse\n1 aurasoma\n1 grande bougie blanche (Led?)\n\nSensitif : \nACHATS : \nTalc hypoallergenique (env . 5 € la boite en pharmaicie X2)\nPlumes : 20€\nApports perso: \nFoulards,\nTissus\nAccessoires \n\nPrimal : \nAchats : Argiles 4 couleurs : \n CATTIER Argile verte ultra-ventilée 750g   : 11€\npuis 3 autres couleurs en paquet de 600 gr : 7 € chaque X3\nMatériel restaurant ou lingerie\n* 8 bols (2 par couleur d'argile)\n* des bouteilles d'eau pour faire les mélange d'argile et humidifier les tissus\n* des linges types carrés de serviettes éponges ou gants de toilette,  pour les mains ou nettoyer une partie du corps si besoin\n* des cuillères pour mélanger les préparations d'argile\n1 bassine de rinçage (eau)\n\nSoundsystem avec 3 micros : Audrey, Paul et Traducteur-ice",
    updatedAt: "2026-08-26 18:36"
  },

  "JOUR3_PREPARATION_RASSEMBLEMENT": {
    html: "",
    text: "",
    updatedAt: "2026-08-25 10:17"
  },

  "JOUR3_THE_SPACE_BETWEEN_US__3HRS_": {
    html: "",
    text: "",
    updatedAt: "2026-08-25 12:27"
  },

  "JOUR3_TRAVERSER___SOUFFLE__FROID_ET_": {
    html: "- Une piscine gonflable<br>- 200KG de glaçons<br>- Un accès à l'eau pour remplir la piscine",
    text: "- Une piscine gonflable\n- 200KG de glaçons\n- Un accès à l'eau pour remplir la piscine",
    updatedAt: "2026-08-26 18:36"
  },

  "JOUR3_VOYAGE_AVEC_LA_ROSE__FEMMES_": {
    html: "- tasses ou gobelets<br>- 2 thermos (pour la tisane de la rose)<br>- 2 plateaux",
    text: "- tasses ou gobelets\n- 2 thermos (pour la tisane de la rose)\n- 2 plateaux",
    updatedAt: "2026-08-26 18:36"
  },

  "JOUR3_YONI___LINGAM_PUJA": {
    html: "Besoin de 2 assistants avant pour la mise en place qui prend un peu de temps(30 min si les assistants sont efficaces). besoin d'acheter des fleurs (peu prendre des fleurs pas chère comme plusieurs bouquets au supermarché) et aussi quelques roses (pour les casser et utiliser juste les pétales)",
    text: "Besoin de 2 assistants avant pour la mise en place qui prend un peu de temps(30 min si les assistants sont efficaces). besoin d'acheter des fleurs (peu prendre des fleurs pas chère comme plusieurs bouquets au supermarché) et aussi quelques roses (pour les casser et utiliser juste les pétales)",
    updatedAt: "2026-08-25 03:37"
  },

  "JOUR3__CALL__POUR_RASSEMBLEMENT": {
    html: "",
    text: "",
    updatedAt: "2026-08-25 10:18"
  },

  "JOUR3__DE_A_KALI": {
    html: "encens + bougies",
    text: "encens + bougies",
    updatedAt: "2026-08-30 00:23"
  },

  "JOUR3____STAND__MAQUILLAGE___": {
    html: "",
    text: "",
    updatedAt: "2026-08-24 01:29"
  },

  "JOUR4_ALICE___DISSOLUTION__2_3__MASS": {
    html: "- 15 bougies electriques (grand modèle)<br>- mouchoirs<br>- 15 Aleses pour matelas <br>- 15 Assiettes en cartons",
    text: "- 15 bougies electriques (grand modèle)\n- mouchoirs\n- 15 Aleses pour matelas\n- 15 Assiettes en cartons",
    updatedAt: "2026-08-29 13:10"
  },

  "JOUR4_BALANCE_CONCERT_KELLY_AURA___C": {
    html: "",
    text: "",
    updatedAt: ""
  },

  "JOUR4_CONCERT_CHANTS___MANTRA_AVEC_K": {
    html: "",
    text: "",
    updatedAt: "2026-08-28 15:49"
  },

  "JOUR4_DANSE_ECSTATIC_CONTACT__2_": {
    html: "",
    text: "",
    updatedAt: ""
  },

  "JOUR4_DANSE_FUSION___GUIDER___SE_LAI": {
    html: "Bandeaux pour les yeux",
    text: "Bandeaux pour les yeux",
    updatedAt: "2026-08-26 18:36"
  },

  "JOUR4_ECSTATIC_DANCE_EN_SILENT_DISCO": {
    html: "- multiprises<br>- rallonge<br>- table haute pour le matériel de djing",
    text: "- multiprises\n- rallonge\n- table haute pour le matériel de djing",
    updatedAt: "2026-08-26 18:36"
  },

  "JOUR4_LA_MERKABA_TANTRIQUE_": {
    html: "",
    text: "",
    updatedAt: ""
  },

  "JOUR4_LOVE_TEMPLE_DEVOTIONNEL": {
    html: "3 grandes bassines (suffisamment larges pour les deux pieds) <br>- 3 Chaises associées <br>- 6 petites serviettes <br>- 3 ou 4 tables basses <br>- 3 grands miroirs pleins pieds <br>- Abondance d'encens, <br>de bougies <br>et de fleurs pour les offrandes <br>- Une grande chaise type \"trône\"",
    text: "3 grandes bassines (suffisamment larges pour les deux pieds) \n- 3 Chaises associées \n- 6 petites serviettes \n- 3 ou 4 tables basses \n- 3 grands miroirs pleins pieds \n- Abondance d'encens, \nde bougies \net de fleurs pour les offrandes \n- Une grande chaise type \"trône\"",
    updatedAt: "2026-08-25 03:37"
  },

  "JOUR4_L_OUROBOROS_TANTRIQUE___CERCLE": {
    html: "",
    text: "",
    updatedAt: ""
  },

  "JOUR4_PREPARATION_RASSEMBLEMENT": {
    html: "",
    text: "",
    updatedAt: "2026-08-25 10:14"
  },

  "JOUR4_RELEASING_SEXUAL_SHAME___CONDI": {
    html: "Pas de limite de participants : plus le groupe est grand, mieux c’est.",
    text: "Pas de limite de participants : plus le groupe est grand, mieux c’est.",
    updatedAt: "2026-08-26 18:36"
  },

  "JOUR4_ROUE_DE_L_AIR___POEMES_MEDECIN": {
    html: "1 Pied de micro",
    text: "1 Pied de micro",
    updatedAt: "2026-08-25 03:37"
  },

  "JOUR4_ROUE_DE_L_EAU___ROUE_DU_CONSEN": {
    html: "diffusion son bluetooth",
    text: "diffusion son bluetooth",
    updatedAt: "2026-08-25 03:37"
  },

  "JOUR4_SACRED_MIRRORS__GRIEF_CEREMONY": {
    html: "Full sound system required for electronic music to drive the energy of ceremony, and mics and system set up for Misch to play live for the last hour. I will also need a sturdy microphone for opening ceremony of this, prefereably with mic stand while I sit so I do not have to hold the mic. Scheduling note: best placed after lunch and before dinner so participants are not carrying a full belly into depth work. Day Three or 4 placement is intentional to allow the earlier ceremonies to have done their work first, and to give ample time after for the remaining days to integrate.",
    text: "Full sound system required for electronic music to drive the energy of ceremony, and mics and system set up for Misch to play live for the last hour. I will also need a sturdy microphone for opening ceremony of this, prefereably with mic stand while I sit so I do not have to hold the mic. Scheduling note: best placed after lunch and before dinner so participants are not carrying a full belly into depth work. Day Three or 4 placement is intentional to allow the earlier ceremonies to have done their work first, and to give ample time after for the remaining days to integrate.",
    updatedAt: "2026-08-25 03:58"
  },

  "JOUR4_THE_ART_OF_KISSING_": {
    html: "1 bain de bouche à diluer<br>1 carafe d’eau<br>30 Gobelets carton  <br>1 rouleau de sopalin<br>1 bassine d’eau (usées)<br>1 tictac<br>1 paquet de Chocolats dragéifiés : type M&amp;M - <br>10 fruits &amp;  1 couteau : bananes, brugnon ou nectarine, pommes et un plateau<br>30 Masques / bandeau<br>9 x 3 pinces métaliques<br>Besoins estimés pour 30 personnes",
    text: "1 bain de bouche à diluer\n1 carafe d’eau\n30 Gobelets carton  \n1 rouleau de sopalin\n1 bassine d’eau (usées)\n1 tictac\n1 paquet de Chocolats dragéifiés : type M&M - \n10 fruits &  1 couteau : bananes, brugnon ou nectarine, pommes et un plateau\n30 Masques / bandeau\n9 x 3 pinces métaliques\nBesoins estimés pour 30 personnes",
    updatedAt: "2026-08-26 18:36"
  },

  "JOUR4__CALL__POUR_RASSEMBLEMENT": {
    html: "",
    text: "",
    updatedAt: "2026-08-25 10:19"
  },

  "JOUR4__DE___LA_LIBERTE__2HRS_": {
    html: "• Sono &amp; table de mix<br>• Micro main sans fil<br>• Plein de cordes. Plein ! Une par personne + staff (je formule une demande directe à Amana pour qu'il me prête les siennes 😻. Ma préférence va à des cordes de Shibari mais cela peut aussi être des cordes qui ne soient pas de véritables cordes de Shibari consacrées…)",
    text: "• Sono & table de mix\n• Micro main sans fil\n• Plein de cordes. Plein ! Une par personne + staff (je formule une demande directe à Amana pour qu'il me prête les siennes 😻. Ma préférence va à des cordes de Shibari mais cela peut aussi être des cordes qui ne soient pas de véritables cordes de Shibari consacrées…)",
    updatedAt: "2026-08-26 18:36"
  },

  "JOUR4____STAND__MAQUILLAGE___": {
    html: "",
    text: "",
    updatedAt: "2026-08-24 01:29"
  },

  "JOUR5_ALCHIMIE_FLUIDE__DANSE_DES_DAU": {
    html: "L'an dernier la piscine etait trop froide. Demander s'il est possible de chauffer la piscine 1 jour avant!<br>- Faire chauffer la piscine (un jour avant)<br>- lunettes de plongée / masques (pour ceux qui n'en ont pas)<br>(pince nez ramené par David)",
    text: "L'an dernier la piscine etait trop froide. Demander s'il est possible de chauffer la piscine 1 jour avant!\n- Faire chauffer la piscine (un jour avant)\n- lunettes de plongée / masques (pour ceux qui n'en ont pas)\n(pince nez ramené par David)",
    updatedAt: "2026-08-26 18:36"
  },

  "JOUR5_ANIMAL_EMBODIMENT_": {
    html: "",
    text: "",
    updatedAt: ""
  },

  "JOUR5_AU_COMMENCEMENT_ETAIT_LE_CORPS": {
    html: "gel désinfectant pour les mains, coupelles pour huile et sopalin<br>25 masques",
    text: "gel désinfectant pour les mains, coupelles pour huile et sopalin\n25 masques",
    updatedAt: "2026-08-26 18:36"
  },

  "JOUR5_CONCERT__MITSCH_KOHN____ARTIST": {
    html: "Enregistreur multipiste pour cet événement. Merci d'en faire la demande à l'ingénieur du son à l'avance afin qu'il puisse préparer l'enregistrement (clé USB, disque dur, ordinateur portable).<br><br>Would be amazing to have a MULTITRACK RECORDING for this event. Please request this for the soundengineer beforehand so he can prepare the recording (stick, HD, Laptop)",
    text: "Enregistreur multipiste pour cet événement. Merci d'en faire la demande à l'ingénieur du son à l'avance afin qu'il puisse préparer l'enregistrement (clé USB, disque dur, ordinateur portable).\n\nWould be amazing to have a MULTITRACK RECORDING for this event. Please request this for the soundengineer beforehand so he can prepare the recording (stick, HD, Laptop)",
    updatedAt: "2026-08-25 03:37"
  },

  "JOUR5_EMBRASSER_LE_MYSTERE": {
    html: "8 à 10 spots (Demander à Felix)",
    text: "8 à 10 spots (Demander à Felix)",
    updatedAt: "2026-08-25 03:37"
  },

  "JOUR5_ET_SI_TU_TISSAIS_LE_FIL_DE_TA_": {
    html: "3 helpers svp<br>- 2 micros sans fils",
    text: "3 helpers svp\n- 2 micros sans fils",
    updatedAt: "2026-08-25 03:37"
  },

  "JOUR5_GLORIOUS_BODIES___CORPS_DE_GLO": {
    html: "-1 bâche plastique de 4mx 6m pour protéger le sol<br>-1 rouleau de bande scotch de marquage tapissier pour scotcher la bâche au sol (minimum 30 m)<br>- 1 bassine d’eau <br>- 1 carafe d’eau<br>Confidentialité / formulaire droits image (Sabryna)<br>- De quoi fixer du tissus tendu au mur :<br>= Scotch type Gaffer, soit des crochets à fixer au mur avec du velcro --",
    text: "-1 bâche plastique de 4mx 6m pour protéger le sol\n-1 rouleau de bande scotch de marquage tapissier pour scotcher la bâche au sol (minimum 30 m)\n- 1 bassine d’eau \n- 1 carafe d’eau\nConfidentialité / formulaire droits image (Sabryna)\n- De quoi fixer du tissus tendu au mur :\n= Scotch type Gaffer, soit des crochets à fixer au mur avec du velcro --",
    updatedAt: "2026-08-26 18:36"
  },

  "JOUR5_GROUNDING___INTEGRATIVE_MOVEME": {
    html: "-",
    text: "-",
    updatedAt: "2026-08-25 03:58"
  },

  "JOUR5_LIVE_ELECTRO_ACCOUSTIQUE___VOI": {
    html: "",
    text: "",
    updatedAt: ""
  },

  "JOUR5_L_ART_DE_DEVENIR_UN_AMANT_D_EX": {
    html: "Les lits sont préférables, mais des chaises feront également l’affaire. J’aimerais également disposer de masques ou de bandeaux, mais ce n’est pas indispensable.",
    text: "Les lits sont préférables, mais des chaises feront également l’affaire. J’aimerais également disposer de masques ou de bandeaux, mais ce n’est pas indispensable.",
    updatedAt: "2026-08-26 18:36"
  },

  "JOUR5_PREPARATION_RASSEMBLEMENT": {
    html: "",
    text: "",
    updatedAt: "2026-08-25 10:14"
  },

  "JOUR5_TANTRA_CAFE___TOUT_CE_QUE_VOUS": {
    html: "-",
    text: "-",
    updatedAt: "2026-08-25 03:58"
  },

  "JOUR5__CALL__POUR_RASSEMBLEMENT": {
    html: "",
    text: "",
    updatedAt: "2026-08-25 10:19"
  },

  "JOUR5__DE___SHIVA_": {
    html: "encens + bougies",
    text: "encens + bougies",
    updatedAt: "2026-08-25 03:58"
  },

  "JOUR5____STAND__MAQUILLAGE____": {
    html: "",
    text: "",
    updatedAt: "2026-08-24 01:30"
  },

  "JOUR6_ATELIER_ARTISTIQUE___FREE_YOUR": {
    html: "Demander à Echo Clem et/ou Felix le materriel suivant:<br>1 micro chant standard avec pied, 1 micro-casque, 1 retour de scène",
    text: "Demander à Echo Clem et/ou Felix le materriel suivant:\n1 micro chant standard avec pied, 1 micro-casque, 1 retour de scène",
    updatedAt: "2026-08-28 15:23"
  },

  "JOUR6_BAIN_MANTRIQUE___SOIN_SONORE_C": {
    html: "enscens / bougies",
    text: "enscens / bougies",
    updatedAt: "2026-08-25 03:58"
  },

  "JOUR6_DANSE_DE_SUBLIMATION_ET_YI_KIN": {
    html: "",
    text: "",
    updatedAt: ""
  },

  "JOUR6_LOVING_LEADERSHIP_": {
    html: "6 spots (voir avec Felix)",
    text: "6 spots (voir avec Felix)",
    updatedAt: "2026-08-25 03:37"
  },

  "JOUR6_L_ART_D_ETRE_EN_MOUVEMENT_": {
    html: "- Sol propre, svp! <br>- Espace ventilé autant que possible avant l'atelier <br>- papier arménie à la Rose<br>(PHOTOS &amp; VIDEO)",
    text: "- Sol propre, svp! \n- Espace ventilé autant que possible avant l'atelier \n- papier arménie à la Rose\n(PHOTOS & VIDEO)",
    updatedAt: "2026-08-26 18:36"
  },

  "JOUR6_PRANAM___DANSE_DEVOTIONNELLE": {
    html: "Micro casque",
    text: "Micro casque",
    updatedAt: "2026-08-25 03:37"
  },

  "JOUR6_PREPARATION_RASSEMBLEMENT": {
    html: "",
    text: "",
    updatedAt: "2026-08-25 10:18"
  },

  "JOUR6_SOIREE_SURPRISE_POUR_ENFLAMMER": {
    html: "",
    text: "",
    updatedAt: ""
  },

  "JOUR6_THE_TEMPLE_OF_EROTIC_INNOCENCE": {
    html: "body paint, and playful toys for ceremony are welcome.",
    text: "body paint, and playful toys for ceremony are welcome.",
    updatedAt: "2026-08-25 03:58"
  },

  "JOUR6_UN_BAIN_DE_GUERISON_CORPS_ET_A": {
    html: "MATOS: 40 bougies à LED (Voir avec Sabryna<br>MATOS: spots puissants qui enveloppent toute la pièce de Lumière violette (Voir avec Felix) - Quantité: 6",
    text: "MATOS: 40 bougies à LED (Voir avec Sabryna\nMATOS: spots puissants qui enveloppent toute la pièce de Lumière violette (Voir avec Felix) - Quantité: 6",
    updatedAt: "2026-08-25 03:37"
  },

  "JOUR6_VOYAGE_AU_PAYS_DE_MON_SEXE": {
    html: "Quatre (4) flacons de talc.<br>- 2 micros sans fil HF mains",
    text: "Quatre (4) flacons de talc.\n- 2 micros sans fil HF mains",
    updatedAt: "2026-08-25 03:37"
  },

  "JOUR6__CALL__POUR_RASSEMBLEMENT": {
    html: "",
    text: "",
    updatedAt: "2026-08-25 10:19"
  },

  "JOUR6____STAND__MAQUILLAGE___": {
    html: "",
    text: "",
    updatedAt: "2026-08-24 01:29"
  },

  "JOUR7_CEREMONIE_DE_CLOTURE": {
    html: "",
    text: "",
    updatedAt: ""
  },

  "LOG_Jour2_CEROMONIE_CACAO___DANCING_HEAR": {
    html: "besoin de <br>- 4 casserols / <br>- 4 louche / <br>- 200 gobelets en carton (3dl) nombre de tasse en fonctions du nombre de participants.<br><br>Besoin de 8 helpers pour servirs le cacao",
    text: "besoin de \n- 4 casserols / \n- 4 louche / \n- 200 gobelets en carton (3dl) nombre de tasse en fonctions du nombre de participants.\n\nBesoin de 8 helpers pour servirs le cacao",
    updatedAt: "2026-08-25 03:37"
  },

  "LOG_Jour2_ECSTATIC_DANCE_NIVEAU_1_": {
    html: "",
    text: "",
    updatedAt: ""
  },

  "LOG_Jour2_MEDITATION_ACTIVE": {
    html: "",
    text: "",
    updatedAt: "2026-08-30 04:45"
  },

  "LOG_Jour2_PLONGEZ_DANS_L_EXPERIENCE___DU_2": {
    html: "- 7 roses rouges<br>- 1 rose blanche <br>- 1 récipient avec de l'eau",
    text: "- 7 roses rouges\n- 1 rose blanche \n- 1 récipient avec de l'eau",
    updatedAt: "2026-08-26 18:36"
  },

  "LOG_Jour2_RASSEMBLEMENT": {
    html: "",
    text: "",
    updatedAt: "2026-08-25 10:13"
  },

  "LOG_Jour2_YOGA2": {
    html: "",
    text: "",
    updatedAt: ""
  },

  "LOG_Jour3_MEDITATION_ACTIVE_D_OSHO___LAU": {
    html: "• Sono &amp; table de mix<br>• Micro main sans fil<br>• Masques à prêter si besoin",
    text: "• Sono & table de mix\n• Micro main sans fil\n• Masques à prêter si besoin",
    updatedAt: "2026-08-26 18:36"
  },

  "LOG_Jour3_YOGA": {
    html: "- bloc de yoga<br>- Je vais le guider en Francais. Avec des petites touches en anglais possible mais pas dans l'entiereté. Donc à communiquer ou voir pour un traducteur??",
    text: "- bloc de yoga\n- Je vais le guider en Francais. Avec des petites touches en anglais possible mais pas dans l'entiereté. Donc à communiquer ou voir pour un traducteur??",
    updatedAt: "2026-08-26 18:36"
  },

  "LOG_Jour4_MEDITATION": {
    html: "Une personne qui puisse traduire les instructions",
    text: "Une personne qui puisse traduire les instructions",
    updatedAt: "2026-08-25 03:37"
  },

  "LOG_Jour4_YOGA": {
    html: "- bloc de yoga<br>- Je vais le guider en Francais. Avec des petites touches en anglais possible mais pas dans l'entiereté. Donc à communiquer ou voir pour un traducteur??",
    text: "- bloc de yoga\n- Je vais le guider en Francais. Avec des petites touches en anglais possible mais pas dans l'entiereté. Donc à communiquer ou voir pour un traducteur??",
    updatedAt: "2026-08-26 18:36"
  },

  "LOG_Jour5_MYSTIC_SHIBARI___L_ART_DE_LA_T": {
    html: "40 Yoga Mats",
    text: "40 Yoga Mats",
    updatedAt: "2026-08-25 03:37"
  },

  "LOG_Jour5_ROUE_DU_FEU___LIBERATION_EMOTI": {
    html: "tambours, mouchoirs, quelques serviettes de toilettes éventuellement.<br>diffusion son bluetooth",
    text: "tambours, mouchoirs, quelques serviettes de toilettes éventuellement.\ndiffusion son bluetooth",
    updatedAt: "2026-08-25 03:58"
  },

  "LOG_Jour5_YOGA_NIDRA": {
    html: "",
    text: "",
    updatedAt: ""
  },

  "LOG_Jour6_ACROYOGA_": {
    html: "",
    text: "",
    updatedAt: ""
  },

  "LOG_Jour6_ALICE___INCARNATION__3_3__MASS_2": {
    html: "* Quelques tables de massage pour les personnes ayant des difficultés à pratiquer confortablement au sol.",
    text: "* Quelques tables de massage pour les personnes ayant des difficultés à pratiquer confortablement au sol.",
    updatedAt: "2026-08-29 13:13"
  },

  "LOG_Jour6_CONFERENCE___POUR_UN_TANTRA_D_": {
    html: "- 2 micros sans fils<br>Besoin d'une traduction de qualité avec quelqu'un de rapide et d'un.e seul.e helper",
    text: "- 2 micros sans fils\nBesoin d'une traduction de qualité avec quelqu'un de rapide et d'un.e seul.e helper",
    updatedAt: "2026-08-25 03:37"
  },

  "LOG_Jour6_CONTACT_MEETS_TANTRA": {
    html: "Micro-casque",
    text: "Micro-casque",
    updatedAt: "2026-08-25 03:37"
  },

  "LOG_Jour6_LA_GRANDE_SPIRALE_DU_COEUR_": {
    html: "enscens / bougies",
    text: "enscens / bougies",
    updatedAt: "2026-08-28 15:23"
  },

  "LOG_Jour6_MINDFULNESS___MOUVEMENTS_SOMAT": {
    html: "- 1 Bol tibétain<br>- papier d'arménie à la rose<br>(PHOTOS &amp; VIDEO)",
    text: "- 1 Bol tibétain\n- papier d'arménie à la rose\n(PHOTOS & VIDEO)",
    updatedAt: "2026-08-26 18:36"
  },

  "LOG_Jour6_MYSTIC_SHIBARI__THE_ART_OF_SUR": {
    html: "40 Yoga Mats",
    text: "40 Yoga Mats",
    updatedAt: "2026-08-25 03:37"
  },

  "LOG_Jour7_YOGA___AUTOMASSAGES": {
    html: "- bloc de yoga<br>- Je vais le guider en Francais. Avec des petites touches en anglais possible mais pas dans l'entiereté. Donc à communiquer ou voir pour un traducteur??",
    text: "- bloc de yoga\n- Je vais le guider en Francais. Avec des petites touches en anglais possible mais pas dans l'entiereté. Donc à communiquer ou voir pour un traducteur??",
    updatedAt: "2026-08-26 18:36"
  },

  "LOG_Jour1_PETIT_DEJ_2": {
    html: "DES OEUFS POUR LES PROT §§§ZZZ",
    text: "DES OEUFS POUR LES PROT §§§ZZZ",
    updatedAt: "2026-08-30 09:49"
  },

  "JOUR1_TESTS_SON___SONO_A__SHIVA": {
    html: "",
    text: "",
    updatedAt: "2026-08-28 20:15"
  },

  "JOUR2_PREPA_TEMPLE__CHENREZIG____SHI": {
    html: "",
    text: "",
    updatedAt: "2026-08-29 16:55"
  },

  "JOUR2_PREPA_CEREMONIE_CACAO__SHIVA__": {
    html: "",
    text: "",
    updatedAt: "2026-08-30 12:29"
  }

};
